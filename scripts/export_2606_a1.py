...
def modify_docx(src: Path, dst: Path, replace_char: str) -> None:
    ...
    for p in doc.paragraphs:
        if paragraph_has_image(p):
            continue
        updated = bump_plus_pattern(p.text)
        if updated != p.text:
            p.text = updated
...
def write_link_file(welcome_pdf: Path) -> None:
    original_text = extract_pdf_text(welcome_pdf)
...
def main() -> None:
    suffix = (sys.argv[1] if len(sys.argv) > 1 else "A1").strip()
    if not suffix:
        suffix = "A1"
